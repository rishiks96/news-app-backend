...
def main():
    ...
    ws = wb.active
    ws.title = "Crypto News"
    
    headers = ["Title", "Published On", "Summary"]
    ws.append(headers)
    
    for title, site, full_text in all_articles:
        try:
            # Clean all text before adding to Excel
            clean_title = clean_text(title.upper())
            clean_site = clean_text(site)
            summary = summarize_text(full_text)
            
            row = [clean_title, clean_site, summary]
            ws.append(row)
        except Exception as e:
            print(f"Warning: Failed to add article to Excel: {title[:50]}... Error: {e}")
            continue
    
    wb.save("crypto_latest_news.xlsx")
    
    print("="*60)
    print("✓ SUCCESS: Saved as crypto_latest_news.xlsx")
