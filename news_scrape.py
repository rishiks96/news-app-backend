...
def llm_explain_summary(title, text):
    prompt = f"""
    Explain this crypto news article in simple language.

    Requirements:
    - About 800–1000 words
    - Multiple paragraphs
    - Explain background of coins, companies, people mentioned
    - Explain what the technology means
    - Explain why it matters
    - Write for a beginner
    - No marketing tone
    - No login/signup text
    - No ads
...
def update_excel_file(all_articles, filename="titles.xlsx"):
    """Update Excel file with titles and sources, replacing previous data"""
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment
    
    print(f"\nUpdating Excel file: {filename}")
    
    # Create new workbook (this replaces any existing file)
    wb = Workbook()
    sheet = wb.active
    sheet.title = "News Articles"
    ...
    # Save the file
    wb.save(filename)
    print(f"✓ Excel file updated with {len(all_articles)} articles")
